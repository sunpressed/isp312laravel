...
@section('body')
    ...
    <x-box>
        ...
        <div class="row">
            ...
            <div class="col-9">{{ $user->birthday->format('d.m.Y') }}
                ({{ round($user->birthday->diffInYears(), 0) }} {{ trans_choice("{0} лет|{1} год|[2,4] года|[5,*] лет", round($user->birthday->diffInYears(), 0 )) }})
    ...
    <x-box>
        ...
        <x-form.input type="password" name="password_confirmation" label="Повторите пароль" placeholder="Новый пароль" :error="$errors->first('password_confirmation')"/>
